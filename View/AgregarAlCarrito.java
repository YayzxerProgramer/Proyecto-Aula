...
public class AgregarAlCarrito extends javax.swing.JFrame {
...
//        UsuarioController.sombraBoton(ButtonVolver);
    ...
    public void botonn(boolean aparece){
        ButtonAgregarProducto.setVisible(aparece);
    }
    ...
    private void initComponents() {
        ...
        ButtonAgregarProducto.setBackground(new java.awt.Color(0, 102, 204));
        ButtonAgregarProducto.setFont(new java.awt.Font("Fira Code Medium", 0, 14)); // NOI18N
        ButtonAgregarProducto.setForeground(new java.awt.Color(0, 0, 0));
        ButtonAgregarProducto.setText("Agregar Producto Al Carrito");
    ...
    if (selectedRow != -1) {
        String nombreProducto = (String) TableProductos.getValueAt(selectedRow, 0);
        TextFieldBuscar.setText(nombreProducto.trim());

        Object productoSeleccionado = null;

        switch (tipoProductoSeleccionado) {
            case "Medicamentos" -> {
                productoSeleccionado = medicamento.get(selectedRow);
            }
            case "Bebidas" -> {
                productoSeleccionado = bebidas.get(selectedRow);
            }
            case "Aseo Personal" -> {
                productoSeleccionado = aseoPersonal.get(selectedRow);
            }
            case "Cuidado Sexual" -> {
                productoSeleccionado = cuidadoSexual.get(selectedRow);
            }
            default -> {
        ...
        if (productoSeleccionado != null) {
            ...
            JOptionPane.showMessageDialog(this, "Producto agregado al carrito correctamente.", "Éxito", JOptionPane.INFORMATION_MESSAGE);
        }
    } else {
        JOptionPane.showMessageDialog(this, "Por favor seleccione un producto de la tabla antes de agregar al carrito.", "Advertencia", JOptionPane.WARNING_MESSAGE);
    } 
